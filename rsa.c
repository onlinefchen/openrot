...
static IrotKey* iavb_parse_key_data(const uint8_t* data, size_t length) {
  rotRSAPublicKeyHeader h;
  IrotKey* key = NULL;
  ...
  if (!rsa_public_key_header_validate_and_byteswap(
          (const rotRSAPublicKeyHeader*)data, &h)) {
    printf("Invalid key.\n");
    goto fail;
  }
  ...
  expected_length = sizeof(rotRSAPublicKeyHeader) + 2 * h.key_num_bits / 8;
  if (length != expected_length) {
    printf("Key does not match expected length.\n");
    goto fail;
  }

  n = data + sizeof(rotRSAPublicKeyHeader);
  rr = data + sizeof(rotRSAPublicKeyHeader) + h.key_num_bits / 8;

  /* Store n and rr following the key header so we only have to do one
   * allocation.
   */
  key = (IrotKey*)(malloc(sizeof(IrotKey) + 2 * h.key_num_bits / 8));
  if (key == NULL) {
    goto fail;
  }
  ...
  key->n0inv = h.n0inv;
  key->n = (uint32_t*)(key + 1); /* Skip ahead sizeof(IrotKey) bytes. */
...
static void ifree_parsed_key(IrotKey* key) {
  free(key);
}

/* a[] -= mod */
static void subM(const IrotKey* key, uint32_t* a) {
  int64_t A = 0;
  uint32_t i;
  for (i = 0; i < key->len; ++i) {
...
/* return a[] >= mod */
static int geM(const IrotKey* key, uint32_t* a) {
  uint32_t i;
  for (i = key->len; i;) {
    --i;
    if (a[i] < key->n[i]) {
...
static void montMulAdd(const IrotKey* key,
                       uint32_t* c,
                       const uint32_t a,
                       const uint32_t* b) {
...
/* montgomery c[] = a[] * b[] / R % mod */
static void montMul(const IrotKey* key, uint32_t* c, uint32_t* a, uint32_t* b) {
  uint32_t i;
  for (i = 0; i < key->len; ++i) {
    c[i] = 0;
  }
 ...
 */
static void modpowF4(const IrotKey* key, uint8_t* inout) {
